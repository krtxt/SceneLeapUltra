"""
Smoke test for UNet with PTv3 backbone.

This test verifies that UNet can be instantiated with PTv3 backbones (ptv3_light, ptv3, ptv3_no_flash)
...
from models.decoder.unet_new import UNetModel


def test_unet_ptv3_instantiation(variant='ptv3_light'):
    """Test UNet + PTv3 instantiation and forward pass with different input configurations."""
    ...
    try:
        ...
        for use_rgb, use_object_mask in test_configs:
            print(f"\n[TEST] UNet + {variant}: use_rgb={use_rgb}, use_object_mask={use_object_mask}")
            ...
            with torch.no_grad():
                cond_dict = model.condition(data)
                data.update(cond_dict)
            ...
            # Clean up
            del model, x_t, ts, data, output, cond_dict
            torch.cuda.empty_cache()
        
        print(f"\n[SUCCESS] UNet + {variant} smoke test passed!")
        return True
        
    except Exception as e:
        print(f"\n[FAIL] UNet + {variant} smoke test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    for variant in variants:
        print(f"\n{'='*60}")
        print(f"Testing UNet + {variant}")
        print(f"{'='*60}")
        results[variant] = test_unet_ptv3_instantiation(variant)
