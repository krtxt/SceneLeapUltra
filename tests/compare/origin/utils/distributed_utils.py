...
def setup_distributed_environment(cfg: DictConfig) -> Dict[str, Any]:
    ...
    if enabled_flag == 'auto':
        ...
        if should_enable:
            ...
            logging.info(f"Auto-disabling distributed training (GPUs: {available_gpus}, Distributed available: {is_distributed_available()}).")
    elif enabled_flag is True:
        if not is_distributed_available():
            raise RuntimeError("Distributed training is not available but was explicitly enabled in the config.")
        if available_gpus < 2:
            logging.warning(f"Forcing distributed training with only {available_gpus} GPU(s) available.")
    ...
    if devices == 'auto':
        ...
        if devices > available_gpus:
            logging.warning(f"Requested {devices} GPUs, but only {available_gpus} are available. Using all available GPUs.")
            dist_config['devices'] = available_gpus
        else:
        ...
        if len(valid_devices) != len(devices):
            logging.warning(f"Some specified GPUs are not available. Using valid devices: {valid_devices}")
    ...
    if strategy in supported_strategies:
        ...
        logging.warning(f"Unknown distributed strategy '{strategy}'. Defaulting to 'ddp'.")
    ...
    dist_config['find_unused_parameters'] = distributed_cfg.get('find_unused_parameters', False)
...
def adjust_batch_size_for_distributed(cfg: DictConfig, dist_config: Dict[str, Any]) -> DictConfig:
    """
...
def adjust_learning_rate_for_distributed(cfg: DictConfig, dist_config: Dict[str, Any]) -> DictConfig:
    """
    ...
    try:
        ...
        logging.warning("Could not find 'model.optimizer.lr' in config. Skipping learning rate scaling.")
        return cfg
...
def get_trainer_kwargs(cfg: DictConfig, dist_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if dist_config['enabled']:
        ...
        if strategy_name == 'ddp':
            ...
            logging.warning(f"Unsupported strategy '{strategy_name}'. Defaulting to 'ddp'.")
...
def log_distributed_info(dist_config: Dict[str, Any]) -> None:
    ...
    if dist_config.get('enabled', False):
        ...
        logging.info(f"  - Sync Batch Norm:  {dist_config.get('sync_batchnorm', False)}")
        logging.info("=" * 50)
    else:
