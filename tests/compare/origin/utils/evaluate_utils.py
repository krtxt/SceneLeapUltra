...
def cal_distance_to_mesh(
    ...
    # Compute SDF
    dis, dis_signs, normals_ts, clst_points_ts = compute_sdf(query_points_op_scale.reshape(-1, 3), face_verts_op_scale)
    ...
    normals_physical = (normals_ts * dis_signs.unsqueeze(1)).reshape(batch_size, n_points, 3) # Normals are unit vectors, their orientation is affected by dis_signs

    if with_closest_points:
        closest_points_physical = (closest_points_op_scale * scale).reshape(batch_size, n_points, 3)
...
def cal_q1(
    ...
    if hand_pose.dim() == 1:
        ...
        raise ValueError(f"hand_pose must be 1D, 2D or 3D tensor, got {hand_pose.dim()}D")

    # Calculate hand pose decomposition for all samples
    rot_type = cfg.get('rot_type') if hasattr(cfg, 'get') else cfg['rot_type']
    global_translation, global_rotation, qpos = hand_model.decompose_hand_pose(hand_pose, rot_type=rot_type)
    ...
    for batch_idx in range(effective_batch_size):
        ...
        for link_name in hand_model.mesh:
            ...
            surface_points = surface_points @ curr_rotation.transpose(-2, -1) + curr_translation.unsqueeze(1)
            ...
            thres_contact = cfg.get('thres_contact') if hasattr(cfg, 'get') else cfg['thres_contact']
        ...
        if np.isnan(contact_points_object_np).any() or np.isnan(contact_normals_np).any():
        ...
        lambda_torque = cfg.get('lambda_torque') if hasattr(cfg, 'get') else cfg['lambda_torque']

        theta = np.linspace(0, 2 * np.pi, m, endpoint=False).reshape(m, 1, 1)
        contact_forces = (contact_normals_np + mu * (np.cos(theta) * u + np.sin(theta) * v)).reshape(-1, 3)
        ...
        wrenches = np.concatenate([
            np.concatenate([
                contact_forces,
                lambda_torque * np.cross(np.tile(contact_points_object_np - origin, (m, 1)), contact_forces)
            ], axis=1),
...
def cal_pen(
    ...
    effective_batch_size = hand_pose.shape[0]
    object_surface_points_expanded = object_surface_points.expand(effective_batch_size, -1, -1)  # [B*num_grasps, N, 3]
    x = (object_surface_points_expanded - global_translation.unsqueeze(1)) @ global_rotation
    ...
    for link_name in hand_model.mesh:
        ...
        if "radius" in hand_model.mesh[link_name]:
            ...
            dis_local_sq, dis_signs, normals_ts, clst_points_ts = compute_sdf(x_local_flat, face_verts)
            dis_local = torch.sqrt(dis_local_sq + 1e-8) * (-dis_signs)
        else:
            batch_size_x, num_points_x = x.shape[0], x.shape[1]
            dis_local = torch.full((batch_size_x * num_points_x,), -float('inf'), device=x_local_flat.device, dtype=x_local_flat.dtype)
    ...
    if distances_per_link:
        distances = torch.max(torch.stack(distances_per_link, dim=0), dim=0)[0]  # [B*num_grasps, N]
        penetration_per_sample = torch.clamp(distances.max(dim=1)[0], min=0.0)  # [B*num_grasps]
    else:
