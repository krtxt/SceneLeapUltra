...
class HandModel:
    ...
    def set_parameters_quat(
        ...
        """
        Set translation, rotation, joint angles, and contact points of grasps
        This version accepts quaternion for rotation instead of rot6d

        Parameters
        ----------
        hand_pose: (B, 3+4+`n_dofs`) torch.FloatTensor
            translation, rotation as quaternion, and joint angles
        contact_point_indices: (B, `n_contact`) [Optional]torch.LongTensor
            indices of contact candidates
        """
        assert len(hand_pose.shape) <= 2
