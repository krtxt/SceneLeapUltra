...
class GraspTaskDecoder(nn.Module):
    ...
    def forward(self, input_dict: Dict[str, Tensor]):
        ...
        point_xyz, point_features, image_features = input_dict["point_xyz"], input_dict["point_feature"], input_dict["image_features"]
        enc_features = self.encoder_to_decoder_projection(point_features).permute(2, 0, 1).contiguous()

        query_embed = self.query_embedding.weight.unsqueeze(1).repeat(1, point_xyz.size(0), 1)  # (N, B, C)
        image_features = image_features.unsqueeze(0).repeat(query_embed.shape[0], 1, 1)
        
    
        enc_pos = self.enc_embedding(point_xyz.transpose(1, 2).contiguous()).permute(2, 0, 1).contiguous()
