...
from models.utils.set_metrics import SetMetricsEvaluator
from models.utils.set_ops import (chamfer_set_loss, pairwise_cost,
                                  sinkhorn_ot_loss)
...
class GraspLossPose(nn.Module):
    def __init__(self, loss_cfg):
        ...
        base_cost_weights = self._to_plain_dict(getattr(loss_cfg, 'cost_weights', {}))
        self.set_cost_weights = {
            key: self._try_float(value)
            for key, value in base_cost_weights.items()
            if self._try_float(value) is not None
        }
            ...
            if cost_override is not None:
                ...
                for key, value in override_dict.items():
                    ...
                    if numeric_value is not None:
                        explicit_weights[key] = numeric_value
                    else:
                        logging.warning(f"[SetLoss] Ignoring non-numeric cost weight '{key}': {value}")
                if explicit_weights:
                    self.set_cost_weights = explicit_weights
        ...
        self.set_metrics_cost_weights = dict(self.set_cost_weights)
        self.set_metrics_evaluator = None
        if self.set_metrics_cfg:
            set_metrics_cfg_dict = self._to_plain_dict(self.set_metrics_cfg)
            metrics_cost_override = self._to_plain_dict(set_metrics_cfg_dict.get('cost_weights')) if set_metrics_cfg_dict.get('cost_weights') is not None else {}
            if metrics_cost_override:
                parsed_metrics_weights = {}
                for key, value in metrics_cost_override.items():
                    numeric_value = self._try_float(value)
                    if numeric_value is not None:
                        parsed_metrics_weights[key] = numeric_value
                    else:
                        logging.warning(f"[SetMetrics] Ignoring non-numeric cost weight '{key}': {value}")
                if parsed_metrics_weights:
                    self.set_metrics_cost_weights = parsed_metrics_weights
            else:
                self.set_metrics_cost_weights = dict(self.set_cost_weights)
            if bool(set_metrics_cfg_dict.get('enable', False)):
                try:
                    self.set_metrics_evaluator = SetMetricsEvaluator(
                        ...
                        cost_weights=self.set_metrics_cost_weights
                    )
        ...
        # Quality metrics configuration (logging only)
        self.quality_metrics_cfg = self._to_plain_dict(getattr(loss_cfg, 'quality_metrics', {}))
        self.quality_metrics_enabled = bool(self.quality_metrics_cfg.get('enable', False))
        metrics_cfg = self._to_plain_dict(self.quality_metrics_cfg.get('metrics')) if self.quality_metrics_enabled else {}
        self.quality_metric_specs = {}
        if self.quality_metrics_enabled and metrics_cfg:
            for name, cfg in metrics_cfg.items():
                cfg_dict = self._to_plain_dict(cfg)
                if not cfg_dict.get('enable', False):
                    continue
                weight = self._try_float(cfg_dict.get('weight', 1.0))
                if weight is None:
                    weight = 1.0
                self.quality_metric_specs[name] = weight
            if not self.quality_metric_specs:
                self.quality_metrics_enabled = False

        self.quality_metric_func_map = {
            "penetration_penalty": self._calculate_penetration_penalty,
            "contact_quality": self._calculate_contact_quality,
        }
    ...
    def compute_quality_metrics(self, pred_dict, batch):
        if not self.quality_metrics_enabled or not self.quality_metric_specs:
            return {}

        metrics = {}
        for name, weight in self.quality_metric_specs.items():
            metric_fn = self.quality_metric_func_map.get(name)
            if metric_fn is None:
                if not hasattr(self, '_quality_metric_missing_warned'):
                    logging.warning(f"[QualityMetrics] Unknown metric '{name}', skipping.")
                    self._quality_metric_missing_warned = True
                continue
            try:
                value = metric_fn(pred_dict, batch)
                if isinstance(value, torch.Tensor):
                    if value.numel() > 1:
                ...
                metrics[name] = torch.tensor(value, device='cpu', dtype=torch.float32)
            except Exception as exc:
                logging.error(f"[QualityMetrics] Failed to compute '{name}': {exc}")
        return metrics
