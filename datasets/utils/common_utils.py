...
try:
    ...
    print(
        "Warning: Open3D library not found. Statistical Outlier Removal "
...
def downsample_point_cloud_other(point_cloud, num_points=100000, object_weight=0.6):
    ...
    if object_points.shape[0] > num_object_points:
        object_indices = np.random.choice(
            object_points.shape[0], num_object_points, replace=False
        )
    ...
    if sampled_point_cloud.shape[0] < num_points:
        additional_indices = np.random.choice(
            sampled_point_cloud.shape[0],
            num_points - sampled_point_cloud.shape[0],
            replace=True,
        )
...
def remove_object_outliers_sor(object_points_xyz, nb_neighbors=20, std_ratio=2.0):
    ...
    if object_points_xyz.shape[0] < nb_neighbors:  # Not enough points for SOR
        print(
            f"Warning: Not enough object points ({object_points_xyz.shape[0]}) "
            f"for SOR with {nb_neighbors} neighbors. Skipping."
        )
        return object_points_xyz
...
def downsample_point_cloud_focused(
    point_cloud,
    num_points_to_sample=10000,
    aabb_padding=0.2,
    object_priority_ratio=0.3,
    ...
    if all_object_points_with_mask.shape[0] == 0:
        ...
        if point_cloud.shape[0] > num_points_to_sample:
            indices = np.random.choice(
                point_cloud.shape[0], num_points_to_sample, 
                replace=False
            )
    ...
    object_points_for_aabb_calc = (
        all_object_points_with_mask  # Default to use all marked object points
    )
    if clean_object_points and OPEN3D_AVAILABLE:
        if (
            all_object_points_with_mask.shape[0] > 0
        ):  # Check if there are any object points to clean
            # Extract XYZ for SOR, keep original full points for later sampling
            object_xyz_for_sor = all_object_points_with_mask[:, :3]
            cleaned_object_xyz = remove_object_outliers_sor(
                object_xyz_for_sor,
            ...
            if (
                cleaned_object_xyz.shape[0] < 10
            ):  # Heuristic: if too few points remain, SOR might have been too aggressive
                print(
                    f"Warning: SOR resulted in very few object points ({cleaned_object_xyz.shape[0]}). Reverting to original object points for AABB calculation."
                )
                ...
                temp_cleaned_for_aabb = np.zeros(
                    (cleaned_object_xyz.shape[0], 4), dtype=point_cloud.dtype
                )
                ...
                print(
                    "Warning: SOR removed all object points. Reverting to original object points for AABB calculation."
                )
                # object_points_for_aabb_calc remains all_object_points_with_mask
    ...
    if object_points_for_aabb_calc.shape[0] == 0:
        print(
            "Error: No object points available (even before/after cleaning) to calculate AABB. Using uncleaned points for AABB."
        )
        ...
        if all_object_points_with_mask.shape[0] == 0:
            ...
            print(
                "Critical Error: No object points at all. Cannot proceed with focused sampling."
            )
            # Fallback to random sampling of the whole scene as a last resort
            if point_cloud.shape[0] > num_points_to_sample:
                indices = np.random.choice(
                    point_cloud.shape[0], num_points_to_sample, replace=False
                )
                return point_cloud[indices]
            return point_cloud
    ...
    object_points_in_roi_mask = get_points_in_aabb(
        all_object_points_with_mask, roi_min_coords, roi_max_coords
    )
    object_points_in_roi = all_object_points_with_mask[object_points_in_roi_mask]

    background_points_in_roi_mask = get_points_in_aabb(
        all_background_points_with_mask, roi_min_coords, roi_max_coords
    )
    background_points_in_roi = all_background_points_with_mask[
        background_points_in_roi_mask
    ]
    ...
    if object_points_in_roi.shape[0] > num_desired_object_pts:
        obj_indices = np.random.choice(
            object_points_in_roi.shape[0], num_desired_object_pts, replace=False
        )
    ...
    num_background_pts_to_sample_now = max(
        0, num_background_pts_to_sample_now
    )  # Ensure non-negative
    ...
    if background_points_in_roi.shape[0] > num_background_pts_to_sample_now:
        if num_background_pts_to_sample_now > 0:
            bg_indices = np.random.choice(
                background_points_in_roi.shape[0],
                num_background_pts_to_sample_now,
                replace=False,
            )
            sampled_background_points = background_points_in_roi[bg_indices]
        else:
            sampled_background_points = np.empty((0, 4), 
    ...
    if not final_sampled_points_list:
        print(
            "Warning: ROI resulted in no points to sample. Consider adjusting AABB padding or checking object mask."
        )
        ...
        if all_object_points_with_mask.shape[0] > 0:
            print(
                "Fallback: Sampling from original object points as ROI was empty/insufficient."
            )
            if all_object_points_with_mask.shape[0] > num_points_to_sample:
                obj_indices = np.random.choice(
                    all_object_points_with_mask.shape[0],
...
def transform_point_cloud(
    points_in_camera_frame, cam_R_model_to_camera, cam_t_model_to_camera
):
    ...
    R_cm = R_mc.T
    t_cm = -np.dot(
        R_mc.T, t_mc.reshape(3, 1)
    ).flatten()  # Ensure t_mc is col vector for matmul, then flatten t_cm
...
def transform_hand_pose_batch(hand_poses, cam_R_m2c, cam_t_m2c):
    ...
    grasp_trans_camera = (
        torch.matmul(cam_R_m2c, translation.unsqueeze(-1)).squeeze(-1) + cam_t_m2c
    )
    grasp_euler_camera = matrix_to_euler_angles(grasp_rot_camera, convention="XYZ")
    return torch.cat(
        (grasp_trans_camera.float(), grasp_euler_camera.float(), qpos), dim=1
    )
