...
def _get_rot_labels(rot_type):
    if rot_type == "quat":
        ...
        print(
            f"Warning: Using generic 'axis_angle_dim_0/1/2' for rot_type 'axis'. Ensure these labels are in your JSON or update mapping."
        )
        return [f"axis_angle_dim_{i}" for i in range(3)]
    elif rot_type == "euler":
        print(
            f"Warning: Using generic 'euler_angle_dim_0/1/2' for rot_type 'euler'. Ensure these labels are in your JSON or update mapping."
        )
...
def norm_hand_pose(hand_pose, rot_type, mode):
    ...
    if isinstance(hand_pose, torch.Tensor):
        hand_t, hand_param, hand_r = hand_pose.split(
            (3, JOINT_ANGLE_DIM, rot_dim), dim=-1
        )
        ...
        hand_t, hand_param, hand_r = np.split(
            hand_pose, [3, 3 + JOINT_ANGLE_DIM], axis=-1
        )
...
def norm_hand_pose_robust(hand_pose, rot_type, mode):
    ...
    if isinstance(hand_pose, torch.Tensor):
        ...
        hand_t, hand_param, hand_r = np.split(
            hand_pose_reshaped, [3, 3 + JOINT_ANGLE_DIM], axis=-1
        )
