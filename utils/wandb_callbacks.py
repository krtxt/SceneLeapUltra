...
class WandbVisualizationCallback(Callback):
    ...
    def on_validation_epoch_end(
        ...
        if (
            ...
            and trainer.current_epoch % self.sample_log_freq == 0
        ):
            self._log_sample_predictions(trainer, pl_module)

    def _log_model_graph(
        self, trainer: pl.Trainer, pl_module: pl.LightningModule
    ) -> None:
        """记录模型架构图"""
        try:
                    ...
                    if hasattr(sample_batch, "keys"):
                        for key in sample_batch:
                            if torch.is_tensor(sample_batch[key]):
                                sample_batch[key] = sample_batch[key].to(
                                    pl_module.device
                                )

                    # 记录模型图
                    trainer.logger.experiment.watch(
                        pl_module, log="all", log_freq=100, log_graph=True
                    )
        ...
        try:
            ...
            for key, value in sample_batch.items():
                if torch.is_tensor(value):
                    ...
                    sample_subset[key] = (
                        value[:num_samples] if hasattr(value, "__getitem__") else value
                    )
    ...
    def _create_prediction_visualization(
        self, batch: Dict[str, Any], predictions: Any, epoch: int
...
class WandbMetricsCallback(Callback):
    ...
    def __init__(self, log_histograms: bool = True, histogram_freq: int = 10):
        """
        初始化指标回调
    ...
    def on_train_epoch_end(
        ...
        if self.log_histograms and trainer.current_epoch % self.histogram_freq == 0:
            self._log_parameter_histograms(pl_module, trainer.current_epoch)

    def _log_parameter_histograms(
        self, pl_module: pl.LightningModule, epoch: int
    ) -> None:
        """记录参数和梯度直方图"""
        try:
            if not (
                hasattr(pl_module.trainer, "logger")
                and hasattr(pl_module.trainer.logger, "experiment")
            ):
                return
            ...
            if histograms:
                ...
                logger.info(
                    f"Logged {len(histograms)} parameter/gradient histograms to WandB"
                )
