...
class HandPhysics:
    ...
    def cal_distance(self, x: torch.Tensor) -> torch.Tensor:
        ...
        model = self._model
        if (
            model.global_translation is None
            or model.global_rotation is None
            or model.current_status is None
        ):
            raise ValueError(
                "Hand parameters are not set. Call set_parameters() first."
            )
        ...
        global_rotation_flat = model.global_rotation.view(
            -1, 3, 3
        )  # (B*num_grasps, 3, 3)
        global_translation_flat = model.global_translation.view(
            -1, 3
        )  # (B*num_grasps, 3)
        x_transformed = (
            x - global_translation_flat.unsqueeze(1)
        ) @ global_rotation_flat

        for link_name in model.mesh:
            matrix = model.current_status[link_name].get_matrix()
            x_local = (x_transformed - matrix[:, :3, 3].unsqueeze(1)) @ matrix[
                :, :3, :3
            ]
            ...
            if "radius" not in model.mesh[link_name]:
                ...
                dis_local_signed = model.mesh[link_name]["radius"] - x_local_flat.norm(
                    dim=1
                )
    ...
    def compute_object_penetration(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_in_hand_frame = (
            x_batched - global_translation_flat.unsqueeze(1)
        ) @ global_rotation_flat

        for link_name in model.mesh:
            matrix = model.current_status[link_name].get_matrix()
            x_local = (x_in_hand_frame - matrix[:, :3, 3].unsqueeze(1)) @ matrix[
                :, :3, :3
            ]
            x_local_flat = x_local.reshape(-1, 3)
            ...
            if "radius" not in model.mesh[link_name]:
                assert "face_verts" in model.mesh[link_name]
                face_verts = model.mesh[link_name]["face_verts"]
                dis_local_values, dis_signs, _, _ = compute_sdf(
            ...
            dis_list.append(
                dis_local_signed.reshape(x_batched.shape[0], x_batched.shape[1])
            )
    ...
    def cal_self_penetration_energy(self) -> torch.Tensor:
        ...
        link_indices = model.global_index_to_link_index_penetration.clone().repeat(
            batch_size, 1
        )

        transforms = torch.zeros(
            batch_size, model.n_keypoints, 4, 4, dtype=torch.float, device=model.device
        )
        ...
        for link_name in model.mesh:
            ...
            current_transform = (
                model.current_status[link_name]
                .get_matrix()
                .unsqueeze(1)
                .expand(batch_size, model.n_keypoints, 4, 4)
            )
        ...
        points_world = points_transformed @ global_rotation_flat.transpose(
            1, 2
        ...
        return (
            E_spen.sum((1, 2)) / 2
        )  # Each distance pair is counted twice, so divide by 2
    ...
    def cal_table_penetration(
        ...
        signed_dist = torch.sum(
            (surface_points - table_pos.unsqueeze(1)) * table_normal.unsqueeze(1),
            dim=-1,
        )
    ...
    def sample_contact_points(
        ...
        finger_possible_contact_point_idxs_list = [
            sum(
                [
                    model.link_index_to_global_indices[link_idx]
    ...
    def decompose_hand_pose(
        hand_pose: torch.Tensor, n_dofs: int, rot_type: str = "quat"
        ...
        assert (
            hand_pose.dim() == 2
        ), "hand_pose must be a 1D or 2D tensor for decomposition."
        ...
        # Use slice constants
        from utils.hand_constants import QPOS_SLICE, ROTATION_SLICE, TRANSLATION_SLICE
